...
def calculate_accuracy(threshold, dist, actual_issame):
    ...
    # result = np.logical_xor(predict_issame, actual_issame)
    # print(result)
...
def calculate_tar_far(threshold, dist, actual_issame):
    ...
    tar = 0 if (n_same == 0) else float(true_accept) / float(n_same)
...
def get_best_threshold(thresholds, ens_dist, actual_issame, distance_metric=0, nrof_folds=10):
    ...
    tpr = np.mean(tprs, 0)
    fpr = np.mean(fprs, 0)
    return tpr[best_threshold_index], fpr[best_threshold_index], accuracy, thresholds[best_threshold_index]